...
class BaseFileHelper
{
    ...
    public static function removeDirectory($dir, $options = [])
    {
        ...
        //         shell_exec("echo $path >> /tmp/teste");

        //         if (getenv('HUMHUB_AWS_S3_BUCKET'))
        //             $bucketName = getenv('HUMHUB_AWS_S3_BUCKET');
        //         else
        //             $bucketName = 'seu-bucket-padrao';
        //         if (getenv('HUMHUB_AWS_S3_BUCKET'))
        //             $bucketName = getenv('HUMHUB_AWS_S3_BUCKET');
        //         else
        //             $bucketName = 'seu-bucket-padrao';
        ...
        //         //     static::unlink($path);
        //         // }
        //         $fileToDelete = "s3://$bucketName".str_replace(getenv('HUMHUB_DIR'), '', $path);
        //         if (is_dir($path)) {
        //             shell_exec("aws s3 rm --recursive $fileToDelete");
        //            // static::removeDirectory($path, $options);
        //         } 
        //     }
        //     closedir($handle);
        ...
        $bucketName = getenv("HUMHUB_AWS_S3_BUCKET");
        $uploadDir = getenv("HUMHUB_AWS_S3_UPLOAD_DIRECTORY");

        $pathToDelete = "s3:\/\/$bucketName\/" . str_replace($uploadDir . "/", '', $dir);
        try {
            shell_exec("echo $pathToDelete >> /tmp/teste");
            shell_exec("aws s3 rm --recursive $pathToDelete");
            return true;
    ...
    public static function unlink($path)
    {
        ...
        shell_exec("echo $path > /tmp/teste");
        shell_exec("echo BUCKET_ATUAL " . getenv('HUMHUB_AWS_S3_BUCKET') . " >> /tmp/teste");

        $bucketName = getenv("HUMHUB_AWS_S3_BUCKET");
        $uploadDir = getenv("HUMHUB_AWS_S3_UPLOAD_DIRECTORY");


        shell_exec("echo \"Path atual\" $path >> /tmp/teste");
        $pathToDelete = "s3:\/\/$bucketName\/" . str_replace($uploadDir . "/", '', $path);
        try {
            shell_exec("echo $pathToDelete >> /tmp/teste");
            shell_exec("aws s3 rm $pathToDelete");
            return true;
        } catch (ErrorException $e) {
            return false;
        }
